...
- **Global scope**: `D:/Github/Payment System/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Github/Payment System/.claude/notes/`)
...
2. Glob `D:/Github/Payment System/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Github/Payment System/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Github/Payment System/.claude/notes/` — works in any directory
