"""Producer for payment.transaction.validated topic — Phase 03.

Mirrors DLQProducer exactly: same Producer config, same _delivery_report
callback, same flush(timeout=5), same retry logic (3 retries, backoff [1, 2, 4]),
same crash-on-exhaustion per D-18.

Published after both INITIATED and VALIDATED state writes succeed and before
Kafka offset commit (D-19). If publish fails the consumer crashes so Docker
...
class ValidatedEventProducer:
    """Publishes ValidatedPaymentEvent dicts to payment.transaction.validated.
    ...
    def publish(self, stripe_event_id: str, validated_event: dict[str, Any]) -> None:
        """Publish a validated event to payment.transaction.validated.
        ...
        Args:
            ...
            validated_event: Dict representation of ValidatedPaymentEvent.
        """
        ...
        for attempt, backoff in enumerate(backoff_seconds, start=1):
            try:
                ...
                logger.info(
                    "validated_event_publish_success",
                    topic=TOPIC,
                ...
                last_exc = exc
                logger.warning(
                    "validated_event_publish_attempt_failed",
                    topic=TOPIC,
                    stripe_event_id=stripe_event_id,
                    attempt=attempt,
                    error=str(exc),
        ...
        logger.critical(
            "validated_event_publish_exhausted",
            topic=TOPIC,
            stripe_event_id=stripe_event_id,
            attempts=3,
        )
