...
Plans:
- [x] 08-01-PLAN.md — Migration 004 (reconciliation_discrepancies) + ReconciliationDiscrepancy ORM + persist_discrepancies DAG task + updated unit tests
- [x] 08-02-PLAN.md — Full dbt project scaffold: 10 models (staging/dimensions/facts/marts) + assert_ledger_balanced singular test + imbalanced_transaction seed
...
**Goal**: Cloud Run services, Cloud SQL, Cloud Memorystore, BigQuery in GCP; GitHub Actions CI/CD pipeline with staging + production environments. One-run Loom demo, ~₹15-20 total credit spend.
...
- [x] 11-02-PLAN.md — Build + push 7 Docker images; provision e2-standard-2 VM; deploy Kafka + Zookeeper + Spark + 3 consumers
...
- [ ] 11-05-PLAN.md — GitHub Actions CI/CD: service account + 4 IAM roles, 5 GitHub secrets, workflow (build+test → push → staging auto → production gate)
- [ ] 11-06-PLAN.md — send_webhook_events.py; 9-step Loom demo run; teardown of ephemeral resources (VM, Cloud SQL, Memorystore)
...
| 11. GCP Deploy + CI/CD | v2.0 | 2/6 | In Progress|  |
